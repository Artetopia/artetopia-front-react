import "../Multistep/Multistep"
const StepSixIcon = () => {
    return (
        
        <div className='circle-step rounded-circle ms-1'>
            <svg className='image-icon-svg d-flex justify-content-center align-items-center m-1' width="34px" height="34px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" strokeWidth="0"></g><g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g><g id="SVGRepo_iconCarrier"> <rect x="4.5" y="6.5" width="15" height="11" rx="1.5" stroke="#E91E63"></rect> <path d="M19.5 9.5H4.5" stroke="#E91E63"></path> <path d="M11.5 12.5H7.5" stroke="#E91E63" strokeLinecap="round"></path> <path d="M10.5 14.5H7.5" stroke="#E91E63" strokeLinecap="round"></path> </g></svg>
            <small className='progress-bar-text'>Pagos</small>
        </div>
        
    )
}

export default StepSixIcon;